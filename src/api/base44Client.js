...
const integrations = {
  Core: {
    async UploadFile({ file, bucket = appParams.storageBucket || 'uploads', folder = 'uploads' }) {
      ...
      if (error) throw error
      const storage_ref = buildStorageRef(bucket, path)
      const file_url = await getSignedFileUrl(bucket, path)
      return { file_url, preview_url: file_url, storage_ref, path, bucket }
    },
