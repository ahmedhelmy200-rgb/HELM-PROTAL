...
export function applyVisualIdentity(settings = {}, resolvedTheme = 'dark') {
  ...
  const primaryDefault = resolvedTheme === 'light' ? '#2563eb' : '#3b82f6';
  const accentDefault = resolvedTheme === 'light' ? '#06b6d4' : '#14b8a6';
  const sidebarDefault = resolvedTheme === 'light' ? '#0f2f63' : '#06142c';
  ...
  root.style.setProperty('--sidebar-foreground', resolvedTheme === 'light' ? '0 0% 98%' : '213 31% 92%');
  root.style.setProperty('--sidebar-border', hexToHsl(mixHex(sidebar, '#ffffff', resolvedTheme === 'light' ? 0.16 : 0.09), '#1f2d48'));
