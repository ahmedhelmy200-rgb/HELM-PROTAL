...
import { format, isToday, isTomorrow, differenceInHours, differenceInDays, isValid } from "date-fns"
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {nextClientSession?.session_date && (
                  <Badge className="hero-metric-chip bg-white/10 text-white border-white/10">
                    ...
                    الجلسة القادمة {getSessionLabel(nextClientSession.session_date)} {safeFmt(nextClientSession.session_date, 'HH:mm')}
                  </Badge>
                )}
              </div>
              <div>
            ...
            {recentInvoices.length === 0 ? (
              ...
              <div className="space-y-3">
                {recentInvoices.map((inv) => {
                  ...
                  const canPay = remaining > 0 && inv.status !== 'مدفوعة' && inv.status !== 'ملغاة'
                  return (
                    <div key={inv.id} className="dashboard-row-card">
                      <div className="flex-1 min-w-0">
                        ...
                        {canPay && <p className="text-xs text-amber-300 mt-0.5">متبقي: {remaining.toLocaleString('ar')} د.إ</p>}
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <StatusBadge status={inv.status} />
                        {canPay && (
                          <a
                            href={`/Payment?token=${btoa(JSON.stringify({id: inv.id, ts: Date.now()})).replace(/=/g,'').replace(/\+/g,'-').replace(/\//g,'_')}`}
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-green-500/20 hover:bg-green-500/30 text-green-300 text-xs font-semibold transition-colors"
                          >
                            💳 ادفع الآن
                          </a>
                        )}
                ...
                {recentDocuments.map((doc) => (
                  <div key={doc.id} className="dashboard-row-card">
                    <div>
                      ...
                      <p className="text-xs text-white/60">{doc.doc_type || 'مستند'} {doc.created_date ? `· ${safeFmt(doc.created_date, 'yyyy/MM/dd')}` : ''}</p>
                    </div>
              ...
              {todayTasks.map(task => (
                <div key={task.id} className="dashboard-row-card">
                  ...
                  <div className="flex items-center gap-2 mr-3 shrink-0">
                    ...
                    {task.due_date && <span className="text-xs text-white/55">{safeFmt(task.due_date, 'HH:mm')}</span>}
                  </div>
                </div>
              ))}
            </div>
              ...
              {upcomingSessions.map(session => (
                <div key={session.id} className="dashboard-row-card">
                  ...
                  <div className="flex items-center gap-2 mr-3 shrink-0">
                    ...
                    <span className="text-xs text-white/55">{safeFmt(session.session_date, 'HH:mm')}</span>
                  </div>
                </div>
              ))}
            </div>
