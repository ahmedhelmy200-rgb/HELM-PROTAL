...
export default function AnimatedBackground({ active = true, intensity = 1, theme = "dark" }) {
  ...
  useEffect(() => {
    ...
    const magneticPointer = { x: 0, y: 0, targetX: 0, targetY: 0, active: false, snapped: false };
    const bursts = [];
    ...
    const onPointerMove = (e) => {
      ...
      pointer.y = e.clientY;
      pointer.active = true;
      if (!magneticPointer.active) {
        magneticPointer.x = e.clientX;
        magneticPointer.y = e.clientY;
      }
      magneticPointer.targetX = e.clientX;
      magneticPointer.targetY = e.clientY;
      magneticPointer.active = true;
    };
    const onPointerLeave = () => { pointer.active = false; magneticPointer.active = false; magneticPointer.snapped = false; };
    const onPointerDown = (e) => {
      ...
      magneticPointer.targetX = e.clientX;
      magneticPointer.targetY = e.clientY;
      if (!isMobile) magneticPointer.active = true;
    };
    ...
    const draw = (t) => {
      ...
      if (!isMobile && pointer.active) {
        ...
        if (closest && minDist < 92) {
          magneticPointer.targetX = closest.x;
          magneticPointer.targetY = closest.y;
          magneticPointer.snapped = true;
        } else {
          magneticPointer.targetX = pointer.x;
          magneticPointer.targetY = pointer.y;
          magneticPointer.snapped = false;
        }
        magneticPointer.x += (magneticPointer.targetX - magneticPointer.x) * 0.085;
        magneticPointer.y += (magneticPointer.targetY - magneticPointer.y) * 0.085;
      } else {
        magneticPointer.x = pointer.x;
        magneticPointer.y = pointer.y;
      }
      const interactionPoint = (!isMobile && pointer.active) ? magneticPointer : pointer;
      ...
      if (pointer.active) {
        ...
        for (let i = 0; i < sparkCount; i++) {
          ...
          const length = 12 + Math.sin(t * 0.006 + i) * 8 + ((!isMobile && magneticPointer.snapped) ? 4 : 0);
          ctx.strokeStyle = isLight ? 'rgba(56,118,255,0.22)' : 'rgba(126,194,255,0.33)';
          ctx.lineWidth = magneticPointer.snapped && !isMobile ? 1.15 : 1;
          ctx.beginPath();
          ctx.moveTo(interactionPoint.x, interactionPoint.y);
          ctx.lineTo(interactionPoint.x + Math.cos(angle) * length, interactionPoint.y + Math.sin(angle) * length);
          ctx.stroke();
        }

        if (!isMobile && magneticPointer.snapped) {
          ctx.beginPath();
          ctx.strokeStyle = isLight ? "rgba(37,99,235,0.26)" : "rgba(125,211,252,0.32)";
          ctx.lineWidth = 1;
          ...
          ctx.beginPath();
          ctx.fillStyle = isLight ? "rgba(37,99,235,0.18)" : "rgba(125,211,252,0.24)";
          ctx.arc(interactionPoint.x, interactionPoint.y, 9, 0, Math.PI * 2);
          ctx.fill();
        }
      }
