...
export default function Invoices() {
  ...
  useEffect(() => {
    const offNew = subscribeAppEvent(APP_SHORTCUT_NEW, ({ page }) => !isClient && page === 'Invoices' && handleCreate());
    const offSearch = subscribeAppEvent(APP_SHORTCUT_SEARCH, ({ page }) => page === 'Invoices' && searchRef.current?.focus());
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map(inv => (
              <InvoiceCard key={inv.id} invoice={inv} onEdit={handleEdit} onDelete={handleDelete} onPrint={handlePrint} onMarkPaid={handleMarkPaid} onSendWhatsApp={handleSendWhatsApp} onSendEmail={handleSendEmail} onSendReminder={handleSendReminder} isClient={isClient} />
            ))}
          </div>
