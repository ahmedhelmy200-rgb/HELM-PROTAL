...
export default function Cases() {
  ...
  useEffect(() => {
    const offNew = subscribeAppEvent(APP_SHORTCUT_NEW, ({ page }) => {
      if (!isClient && page === 'Cases') openCreate();
    });
    const offSearch = subscribeAppEvent(APP_SHORTCUT_SEARCH, ({ page }) => {
      if (page === 'Cases') searchRef.current?.focus();
    });
