...
export default function Documents() {
  ...
  useEffect(() => {
    const offNew = subscribeAppEvent(APP_SHORTCUT_NEW, ({ page }) => page === 'Documents' && openCreate());
    const offSearch = subscribeAppEvent(APP_SHORTCUT_SEARCH, ({ page }) => page === 'Documents' && searchRef.current?.focus());
              ...
              <>
                <div className="grid grid-cols-1 gap-3">
                  {filteredDocs.map(doc => (
                    <DocCard key={doc.id} doc={doc} onClick={openEdit} searchQuery={ocrSearch ? search : ""} />
                  ))}
                </div>
